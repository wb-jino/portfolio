...
var Common = function () {
	...
	var handleElement = function(){
		...
		jQuery(document).on("click", ".add_btn", function(event){
			...
			if(iCnt <= iCnt_total){
				...
				if(option == "btn"){
					...
					option += '			<span class="el_name_holder" data_default_name="Button">Button</span>'
					option += '			<div class="el_name_editor">'
					option += '				<input type="text" name="el_name_editor" id="el_name_editor" class="validate" value="Button">'
					option += '				<a href="#" class="el_name_save" title="save"><i class="fa fa-floppy-o" aria-hidden="true"></i></a>'
					option += '			</div>'
					option += '		</div>';
					...
					option += '		<div class="transformarBox_tit">';
					option += '			<i class="fa fa-hand-o-up" aria-hidden="true"></i>'
					option += '			<span class="el_name_holder" data_default_name="Text Block">Text Block</span>'
					option += '			<div class="el_name_editor">'
					option += '				<input type="text" name="el_name_editor" id="el_name_editor" class="validate" value="Text Block">'
					option += '				<a href="#" class="el_name_save" title="save"><i class="fa fa-floppy-o" aria-hidden="true"></i></a>'
					option += '			</div>'
					option += '       </div>'
		...
		jQuery(document).on("click", "#spb .element_delete", function(b) {
			b.preventDefault();
			var a = '<div class="modal_wrapper"><div id="modal1" class="modal modal_spb"><div class="modal-content">';
		...
		jQuery(document).on("click", "#spb .element_clone", function(l){
			l.preventDefault();
		...
		jQuery(document).on("click", ".el_name_holder", function() {
			jQuery(this).parent().next(".transformarBox_controls, .fadeBox_controls").removeClass("active");
			var i = jQuery(this).next().find("#el_name_editor").first();
			jQuery(this).hide();
			jQuery(this).next().show();
			i.focus();
		});

		jQuery(document).on("click", ".el_name_save", function(i) {
			var j;
			i.preventDefault();
			if (jQuery(this).parent().prev().hasClass("el_name_holder")) {
				j = jQuery(this).parent()
			} else {
				j = jQuery(this).parent().parent()
			}
			j.prev().html(jQuery(this).prev().val());
			j.prev().show();
			j.hide()
		});
	}
