	...
	var handleElement = function(){
		...
		jQuery(document).on("click", ".el_add_btn", function(){
			if(jQuery(this).hasClass("el_add_to_section")){
				box_id = jQuery(this).closest(".el_page_box").attr("data-element-num");
				isSection = true;
			}else{
				isSection = false;
			}
			...
			if(iCnt <= iCnt_total){
				...
				jQuery.ajax({
					...
					success : function(data){
						...
						data.attr("data-element-num", "el_box"+iCnt+"");
						if(isSection){ //section 안에 element 추가시
							jQuery("[data-element-num='"+box_id+"']").find(".page_content").append(data);
						}else{
		...
		jQuery(document).on("click", ".element_delete", function(b) {
			...
			el_num = jQuery(this).parents(".el_wrap").attr("data-element-num");
		});

		// element 삭제
		jQuery(document).on("click", ".modal-delete-element .modal-ok-button", function() {
			var $el_box = jQuery("[data-element-num='"+el_num+"']").find(".el_wrap "),
			...
			jQuery("[data-element-num='"+el_num+"']").remove();
			jQuery(".modal_wrapper").remove();
		...
		jQuery(document).on("click", ".element_clone", function(l){
			l.preventDefault();
			var el_clone_num = jQuery(this).parents(".el_wrap").attr("data-element-num");
			var $el_clone_box = jQuery("[data-element-num='"+el_clone_num+"']").find(".el_wrap "),
				  clone_box_size = $el_clone_box.size();
		...
		jQuery(document).on("click", ".element_edit", function(e){
			...
			jQuery(this).parent().siblings(".el_content").find(".el_txt").jqte();
		...
		jQuery(document).on("click", ".element_save", function(e){
			...
			if(!jqteStatus){
				jQuery(this).parent().siblings(".el_content").find(".el_txt").jqte({"status" : jqteStatus});
				jqteStatus = true;
			}
